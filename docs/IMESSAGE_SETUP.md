...
PYTHONPATH=src python3 src/cli_agent_gateway/channels/imessage_fetch.py

# Send one message to configured chat
PYTHONPATH=src SMS_TO='<fallback-id>' \
  python3 src/cli_agent_gateway/channels/imessage_send.py 'gateway test message'
```

## 4. Run gateway

```bash
PYTHONPATH=src python3 -m cli_agent_gateway.app.main /path/to/agent-workdir
```
