...
func (a *Adapter) Fetch() ([]core.InboundMessage, error) {
	if strings.HasPrefix(strings.TrimSpace(a.FetchCmd), "builtin:") {
		return nil, fmt.Errorf("unsupported builtin fetch in Go runtime: %s", a.FetchCmd)
	}
	out, err := runShell(a.FetchCmd, nil, a.FetchTimeoutSec)
	if err != nil {
		return nil, err
	}
	...
	for _, node := range payload {
		id := sanitize(anyToString(node["id"]))
		sender := sanitize(anyToString(node["from"]))
		text := sanitize(anyToString(node["text"]))
		ts := sanitize(anyToString(node["ts"]))
		threadID := sanitize(anyToString(node["thread_id"]))
		if text == "" {
			continue
		}
		if id == "" {
			h := sha256.Sum256([]byte(sender + "|" + text + "|" + ts))
		...
		result = append(result, core.InboundMessage{
			...
			Text:     text,
			TS:       ts,
			Channel:  nonEmpty(a.ChannelID, "command"),
			ThreadID: threadID,
...
func (a *Adapter) Send(text, to, messageID, reportFile string) error {
	...
	cmd := strings.TrimSpace(a.SendCmd)
	if cmd == "" {
		return errors.New("empty send cmd")
	}
	fullCmd := cmd + " " + shellQuote(text)
	_, err := runShell(fullCmd, env, a.SendTimeoutSec)
	return err
}
